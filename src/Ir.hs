...
import Control.Monad.Reader (MonadReader(local), Reader, ReaderT, reader, runReader)
...
collectOverTerms inUTerm inKTerm e =
    case e of
...
mapTermsInExpr mapUTerm mapKTerm e =
    case e of
        KCall {kc_cont, kc_arg} -> do
...
simplify UCall {uc_func = (ULambda Lambda {l_args, l_cont, l_body}), uc_args, uc_cont} =
    runReader
        (substInExprK
             (runReader
                  (substInExprU l_body)
                  (mapToFn $ Map.fromList $ zip (map bindingName l_args) uc_args)))
        (mapToFn $ Map.singleton (bindingName l_cont) uc_cont)
...
simplify Select { sl_field
                , sl_record = LiRecord fields
                , sl_cont = KLambda {kl_arg = Just argName, kl_body}
                }
    | Map.member sl_field fields =
        runReader
            (substInExprU kl_body)
            (mapToFn $ Map.singleton (bindingName argName) $ fields Map.! sl_field)
-- so long as we have no side-effects
simplify UCall {uc_func = _, uc_args = _, uc_cont = KLambda {kl_arg = Nothing, kl_body}} = kl_body
